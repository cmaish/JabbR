...
(function(){
	...
	var drawFavicon = function(label, colour, additionalData) {
		// fallback to updating the browser title if unsupported
        var faviconSupported = getCanvas().getContext && !browser.ie && !browser.safari;
		if (!faviconSupported || options.fallback === 'force') {
            if (options.fallback !== false && typeof options.fallbackTitle === 'function') {
                document.title = options.fallbackTitle(originalTitle, label, !faviconSupported, additionalData);
            }
	...
	Tinycon.setBubble = function(label, colour, additionalData) {
		label = label || '';
		drawFavicon(label, colour, additionalData);
		return this;
	};
